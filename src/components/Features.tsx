  ...
  return (
    <>
      ...
      <Container className="w-[78rem] max-w-[90%] text-center">
        <p className="mx-auto my-16 text-xl leading-tight text-white md:w-full md:text-3xl">
          {text}
        </p>
        {btnText && (
          <Button
    ...
    <Container>
      <div className="mb-[14rem] grid w-full grid-cols-2 place-items-start gap-y-9 text-sm text-gray-300 md:grid-cols-3 md:place-items-center md:text-md">
        {features.map(({ title, text, icon: Icon }) => (
          <div
            key={title}
            className="w-full space-y-1 md:max-w-[25.6rem] [&_svg]:mb-[4px] [&_svg]:fill-white md:[&_svg]:mb-[2px] md:[&_svg]:mr-[6px] md:[&_svg]:inline"
          >
            <Icon />
    ...
    <Container>
      <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-2">
        {features.map(({ title, text, image, imgClassName }) => (
          <div
            key={title}
            className="relative aspect-[1.1/1] overflow-hidden rounded-[2.4rem] border border-gray-100 bg-[radial-gradient(ellipse_at_center,rgba(var(--feature-color),0.15),transparent)] px-8 py-6 before:pointer-events-none before:absolute before:inset-0 before:bg-glass-gradient md:rounded-[4.8rem] md:p-14"
          >
